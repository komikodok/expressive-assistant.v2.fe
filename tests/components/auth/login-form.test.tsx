...
describe('login-form.tsx', () => {
    ...
    test("should render", async () => {
        const form = await screen.getByRole('form', { name: /login-form/i })

        expect(form).toBeInTheDocument()
    })

    test("should update input values when user types email and password", async () => {
    ...
    test("should be redirect to register page when signup button is clicked with router.replace()", async () => {
        const signUpButton = await screen.getByText(/sign up/i)
